...
import { CubeCamera, LoadingManager, PlaneGeometry, Texture } from "three";
// 导入轨道控制器
...
event.onProgress = (url, num, total) => {
  ...
  const percentValue = ((num / total) * 100).toFixed(2) + "%";
  div.innerHTML = percentValue;
};
...
const standardMaterial = new THREE.MeshStandardMaterial({
  ...
  displacementMap: doorHeightTexture, // 导入置换贴图（凹凸）
  displacementScale: 0.1, // 置换贴图缩放值
